...
public class ProductCategoryManagementController {
    ...
    private Map<String, Object> addProductCategorys(@RequestBody List<ProductCategory> productCategoryList, HttpServletRequest request) {
        ...
        if (productCategoryList != null && productCategoryList.size() > 0) {
            try {
                ProductCategoryExecution pe = service.batchAddProductCategory(productCategoryList);
    ...
    private Map<String, Object> removeProductCategory(Long productCategoryId, HttpServletRequest request) {
        ...
        if (productCategoryId != null && productCategoryId > 0) {
            try {
                Shop curShop = (Shop) request.getSession().getAttribute("currentShop");
                ProductCategoryExecution pe = service.deleteProductCategory(productCategoryId, curShop.getShopId());
                if (pe.getState() == ProductCategoryStateEnum.SUCCESS.getState()) {
                    modelMap.put("success", true);
                } else {
                    modelMap.put("success", false);
